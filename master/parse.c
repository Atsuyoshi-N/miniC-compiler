...
// Find a variable or typedef by name.
...
static Type *struct_decl(void) {
  ...
  // foo as a ppointer to an unnamed incomplete struct type.
  if (!consume("{"))
    return struct_type();
...
//
// A gvar-initilizer represents an initialization expression for
...
//
// A constant expreeession is either just a number or ptr+n where ptr
